...
- 项目依赖以 `pyproject.toml` 为权威；刷新依赖时在仓库根目录运行 `uv pip install -e .`。
...
1. 实现 config 加载。
   - 验证：`uv run python -m pytest tests/test_config.py` 检查默认值和 paper config。
...
- smoke training run 输出有限 losses 和 checkpoints；smoke evaluation 输出 ratemaps。
